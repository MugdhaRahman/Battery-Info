...
import com.mrapps.batteryinfo.chart.model.DataPoint
 ...
 */
class LiveChartTouchOverlay(context: Context, attrs: AttributeSet?) : FrameLayout(context, attrs) {
    ...
    private val overlay = LayoutInflater.from(context)
        .inflate(
            R.layout.livechart_touch_overlay,
            this,
            false
        )
     ...
     */
    private var touchListener: LiveChart.OnTouchCallback? = null
    ...
    init {
        ...
        context.theme?.obtainStyledAttributes(
            ...
            try {
                // Color attrs,
                chartStyle.overlayLineColor = getColor(
                    R.styleable.LiveChart_overlayLineColor,
                    chartStyle.overlayLineColor
                )
                chartStyle.overlayCircleColor = getColor(
                    R.styleable.LiveChart_overlayCircleColor,
                    chartStyle.overlayCircleColor
                )
                chartStyle.overlayCircleDiameter = getDimension(
                    R.styleable.LiveChart_overlayCircleDiameter,
    ...
    @Suppress("UNUSED")
    fun setOnTouchCallbackListener(listener: LiveChart.OnTouchCallback): LiveChartTouchOverlay {
        ...
        this.post {
            ...
            try {
                if (initialXPosition > -1f) {
                    ...
                    val coordinates =
                        pathCoordinates[round(pathCoordinates.size.div(2).toFloat()).toInt()]
                    overlay.x = coordinates[0] - (chartStyle.overlayCircleDiameter / 2)
                    overlayPoint.y = coordinates[1] - (chartStyle.overlayCircleDiameter / 2)
                }
            } catch (e: Exception) {
